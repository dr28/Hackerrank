/*
You are planning production for an order. You have a number of machines that each have a fixed number of days to produce an item. Given
that all the machines operate simultaneously, determine the minimum number of days to produce the required order.

For example, you have to produce goal = 10 items. You have three machines that take machines = [2, 3, 2] days to produce an item. The
following is a schedule of items produced:
...
Complete the minimumTime function in the editor below. It should return an integer representing the minimum number of days required to
complete the order.
...
public class MinimumTimeRequired {
    static long minTime(long[] machines, long goal) {
        ...
        return calculateMinTime(machines, goal, 1, machines[machines.length-1]*goal);

    }

    static long calculateMinTime(long[] machines, long goal, long minDays, long maxDays) {

        if (machines.length == 1) return maxDays;
        ...
        while (minDays != maxDays) {
            ...
            for (int i = 0; i < machines.length; i++)
                production += Math.floorDiv(mid, machines[i]);
    ...
    public static void main (String args[]) {
       ...
       System.out.print(minTime(machines, goal));
